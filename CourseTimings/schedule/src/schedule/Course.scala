...
case class Course(id: Int, name: String, instructor: Faculty) {
  ...
  def mailcmd =
    s"""cat $filename | mail ${instructor.username}@math.iisc.ernet.in -s "Course timing preferences" """
...
object Course {
  ...
  def get(fac: Faculty)(implicit l: List[Course]) =
    (l find (_.instructor == fac)).get

  def getOpt(id: Int)(implicit l: List[Course]) = (l find (_.id == id))

  def getOpt(fac: Faculty)(implicit l: List[Course]) =
    (l find (_.instructor == fac))
